...
export default function AuctionManagementPage() {
  ...
  const {
    ...
    isPending,
    refetch,
  } = useGetAllAuctionRequest(0, 0, Number(status));
          ...
          {isPending ? (
            ...
            <>
              <Table className="min-w-full">
                ...
                <TableBody>
                  {data?.result.items.map((auc) => (
                    <TableRow key={auc.auctionId} className="hover:bg-gray-100">
                      <TableCell>
                        {auc.createByAccount?.firstName}{" "}
                        {auc.createByAccount?.lastName}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {auc.minimumBid.toLocaleString()} VND
                      </TableCell>

                      <TableCell>
                        {dayjs(auc.startDate).format("DD/MM/YYYY HH:mm:ss")}
                      </TableCell>
                      <TableCell>
                        {dayjs(auc.endDate).format("DD/MM/YYYY  HH:mm:ss")}
                      </TableCell>
                      <TableCell>
                        <Popover>
                          <PopoverTrigger>
                          ...
                          <PopoverContent className="grid gap-2 w-fit">
                            {auc.statusId === 0 && (
                              <Button
                                ...
                                className="w-fit"
                                onClick={() =>
                                  handleApprove(auc.auctionRequestId)
                                }
                              >
                                Duyệt
                              </Button>
